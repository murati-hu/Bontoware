...
Begin VB.Form kimeneti_alt 
   ...
   End
End
Attribute VB_Name = "kimeneti_alt"
...
Private Sub Form_Resize()
    On Error Resume Next
        ...
        Label5(0).Move Me.ScaleWidth - 4000, Me.ScaleHeight - 400
...
Private Sub Frissit()
    ...
    Do While datum <= ig.Value
        ...
        p = "SELECT Sum(raktarkeszlet.suly) AS SumOfsuly, szamla.kelt "
        p = p & "FROM szamla INNER JOIN (raktarkeszlet INNER JOIN elkelt ON raktarkeszlet.id = elkelt.tetel) ON szamla.id = elkelt.szamla "
        'p = p & "Where (((raktarkeszlet.tipus) = 0) And ((szamla.kelt) > #7/23/2005#) And ((szamla.kelt) < #8/23/2005#)) "
        'p = p & "Where (((raktarkeszlet.tipus) = 0) And ((szamla.kelt) >= " & DatumAtir(tol.Value) & ") And ((szamla.kelt) <= " & DatumAtir(ig.Value) & ")) "
        p = p & "Where (((raktarkeszlet.tipus) = 0) And ((szamla.kelt) = " & DatumAtir(datum) & ")) "
        p = p & "GROUP BY szamla.kelt"
        ...
        If Not Sor.EOF Then
            ...
            'SID = sor.Fields(2)
        Else
            nap = datum 'datum.Year & "." & datum.Month & "." & i & "."
        End If
        Sor.Close
        ...
        p = p & "HAVING (((szamla.kelt)=" & DatumAtir(datum) & ") AND ((raktarkeszlet.auto)=-1) AND ((raktarkeszlet.tipus)=1))"
        ...
        If Not Sor.EOF Then
            ...
            Do While Not Sor.EOF
                ...
                Set elem = alkatreszek.ListItems.Add(, , Sor.Fields(0)) 'Datum, rejtett
                    ...
                    elem.ListSubItems.Add , , datum 'i & "."                                           'Honap hanyadik napja
    ...
    p = p & "WHERE (((szamla.kelt)<#" & tol.Month & "/" & tol.Day & "/" & tol.Year & "#) AND ((raktarkeszlet.elkelt)=True))"
    SQL_p p, Sor
    If Not Sor.EOF Then
        Sor.MoveFirst
    ...
    p = "SELECT Sum(raktarkeszlet.suly) AS SumOfsuly "
    p = p & "FROM raktarkeszlet INNER JOIN (szamla INNER JOIN elkelt ON szamla.id = elkelt.szamla) ON raktarkeszlet.id = elkelt.tetel "
    p = p & "WHERE (((szamla.kelt)<" & DatumAtir(tol.Value) & ") AND ((raktarkeszlet.tipus)=1))"
